...
public class Console2 
{
	...
	public static int readIntBetween(String msg, String errMsg, int min, int max)
	{
		...
		{
			Console2.print(msg);
			res = Console2.readInt();
			if (res < min || res > max)
				Console2.print(errMsg);
	...
	public static String readStringBetween(String msg, String errMsg, String...values)
	{
	    ...
	    do
	    {
	        Console2.print(msg);              // Stampa il messaggio iniziale
	        res = Console2.readString();      // Legge l'input dell'utente
	        
	        if(!v.contains(res))             // Se la risposta NON è nella lista
	            Console2.print(errMsg);       // Stampa il messaggio di errore
	...
	public static String askNotEmptyString(String message, String errorMessage)
    {
        ...
        do
        {
            Console2.print(message);
            res = Console2.readString();
            if(res.isBlank())
                Console2.print(errorMessage);
        }while(res.isBlank());
        return res;
    }
