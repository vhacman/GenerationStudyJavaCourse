...
public class RestaurantAPI
{
    ...
    {
        Optional<Restaurant> restaurantOpt = restaurantRepo.findByIdWithCity(id);
    ...
    public ResponseEntity<Object> newRestaurant(@RequestBody RestaurantDTO dto)
    {
        ...
        return ResponseEntity.status(201).body(restaurantDTOMapper.toDTO(restaurant));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Object> deleteRestaurant(@PathVariable("id") int id)
    {
        Optional<Restaurant> restaurantOpt = restaurantRepo.findById(id);
        if (restaurantOpt.isEmpty())
            return ResponseEntity.notFound().build();
        if (restaurantRepo.hasDeliveries(id))
        {
            List<String> errors = new ArrayList<>();
            errors.add("Cannot delete restaurant: has associated deliveries");
            return ResponseEntity.badRequest().body(errors);
        }
